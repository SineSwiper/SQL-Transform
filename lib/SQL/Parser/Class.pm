...
###   properties.
### * (is => 'rw'), since SQL::Translator works the same way.
...
sub typedef_struct ($$) {
   ...
   MooX::Types::MooseLike::register_types([{
      ...
      test       => sub { blessed($_[0]) and $_[0]->isa("SQL::Translator::Statement::$subpackage") },
      message    => sub { "Object is not a SQL::Translator::Statement::$subpackage class!" },
   }], __PACKAGE__);
   
   # package scoping
   eval "package SQL::Translator::Statement::$subpackage;";
...
### This WHOLE thing is a BEGIN block so that definitions are executed as 
