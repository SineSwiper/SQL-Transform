...
sub clean_hash {
   ...
   foreach my $k (keys %$hash) {
      my $v = ref $hash->{$k} ? dclone $hash->{$k} : $hash->{$k};
      delete $hash->{$k} if (
         !defined $v or
...
sub parse {
   ...
   for (my $i = 0; $i < @$body; $i++) {
      ...
      if (@$rhs <= 2) {
         for (my $j = 0; $j < @$rhs; $j++) {
            my $choice = $rhs->[$j];
            $choice = [ $choice ] unless (ref $choice eq 'ARRAY');
            ...
            if ($qual eq '2+') {
               for (my $k = 0; $k < @$rhs; $k++) {
                  my $kchoice = $rhs->[$k];
                  $kchoice = [ $kchoice ] unless (ref $kchoice eq 'ARRAY');
   ...
   foreach my $rule (@$body) {
      ...
      if (@$choices == 1) {
         if ($o && $choices->[0] =~ /\s+/)
              { $pgx->say( "$lhs: ( ".$choices->[0].' )?' ); }
         else { $pgx->say( "$lhs: ".$choices->[0].($o ? '?' : '') ); }
      }
      else {
         $pgx->print( "$lhs:".($o ? ' (' : '')."\n     " );
         $pgx->say( join "\n   | ",  @$choices );
