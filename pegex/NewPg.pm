...
use base 'SQL::Translator::Lexer::Pg';
...
### New gotrule with different kind of wrapping ###
sub gotrule {
   my $self = shift;
   @_ || return ();
   return [ $_[0] ] if ($self->{parser}{parent}{-wrap} && !ref $_[0]);
   return $_[0];
}
...
sub got_CreateRoleStmt {
   SQL::Translator::Statement::CreateRole->new(
...
sub got_OptRoleList      { $_[0]->lappend($_[1]) }
sub got_AlterOptRoleList { $_[0]->lappend($_[1]) }
...
sub got_AlterOptRoleElem_1 { $_[0]->makeDefElem("password",            $_[1] ) }
sub got_AlterOptRoleElem_2 { $_[0]->makeDefElem("password",            'NULL') }
sub got_AlterOptRoleElem_3 { $_[0]->makeDefElem("encryptedPassword",   $_[1] ) }
sub got_AlterOptRoleElem_4 { $_[0]->makeDefElem("unencryptedPassword", $_[1] ) }
sub got_AlterOptRoleElem_5 { $_[0]->makeDefElem("inherit",             'TRUE') }
sub got_AlterOptRoleElem_6 { $_[0]->makeDefElem("connectionlimit",     $_[1] ) }
sub got_AlterOptRoleElem_7 { $_[0]->makeDefElem("validUntil",          $_[1] ) }
sub got_AlterOptRoleElem_8 { $_[0]->makeDefElem("rolemembers",         $_[1] ) }
...
### CreateOptRoleElem_1: SYSID Iconst
### CreateOptRoleElem_2: ADMIN name_list
### CreateOptRoleElem_3: ROLE name_list
### CreateOptRoleElem_4: IN ROLE name_list
### CreateOptRoleElem_5: IN GROUP name_list
sub got_CreateOptRoleElem_1 { $_[0]->makeDefElem("sysid",        $_[1]) }
sub got_CreateOptRoleElem_2 { $_[0]->makeDefElem("adminmembers", $_[1]) }
sub got_CreateOptRoleElem_3 { $_[0]->makeDefElem("rolemembers",  $_[1]) }
sub got_CreateOptRoleElem_4 { $_[0]->makeDefElem("addroleto",    $_[1]) }
sub got_CreateOptRoleElem_5 { $_[0]->makeDefElem("addroleto",    $_[1]) }
...
sub got_AlterRoleStmt {
   SQL::Translator::Statement::AlterRole->new(
...
sub got_AlterRoleSetStmt {
   SQL::Translator::Statement::AlterRoleSet->new(
      role     => $_[1]->[0],
      database => $_[1]->[1],
      setstmt  => $_[1]->[2],
   );
...
sub got_DropRoleStmt {
   SQL::Translator::Statement::DropRole->new(
      stmt_type  => ROLESTMT_ROLE,
      missing_ok => $_[1]->[0] ? TRUE : FALSE,
      roles      => $_[1]->[1],
   );
}
...
sub got_AlterGroupStmt {
   SQL::Translator::Statement::AlterRole->new(
      role    => $_[1]->[0],
      action  => $_[1]->[1],
      options => $_[0]->lappend( $_[0]->makeDefElem("rolemembers", $_[1]->[2]) ),
   );
}
sub got_add_drop_add  { +1 }
sub got_add_drop_drop { -1 }
...
sub got_CreateSchemaStmt {
   SQL::Translator::Statement::CreateSchema->new(
      #* One can omit the schema name or the authorization id.
      #* ...but not both
      schemaname => $_[1]->[0] || $_[1]->[1],
      authid     => $_[1]->[1],
      schemaElts => $_[1]->[2],
   );
}
...
sub got_set_type_local   { $_[1]->is_local(1); $_[1] }
sub got_set_type_session { $_[1]->is_local(0); $_[1] }

sub got_set_rest { SQL::Translator::Statement::VariableSet->new(@{$_[1]}) }

sub got_set_rest_trans   { (
   kind => VAR_SET_MULTI,
   name => $_[1]->[0] ? 'SESSION CHARACTERISTICS' : 'TRANSACTION',
   args => $_[1]->[1],
) }
...
sub got_set_rest_more_6  { (
   kind => VAR_SET_VALUE,
   name => 'search_path',
   args => $_[0]->lappend($_[0]->makeStringConst($_[1], $_[0]->YYLLoc($_[1], 1))),
) }
...
sub got_set_rest_more_11 { (
   ...
   args => [ $_[0]->makeStringConst($_[1], $_[0]->YYLLoc($_[1], 1)) ],
) }

sub got_var_name { join('.', @{$_[1]}) }
...
sub got_zone_value_2 {
   my $self = shift;
   my ($t, $int, $str, $interval) = @{ shift };
   if (defined $interval) {
      if (($interval->[0]->val & ~(INTERVAL_MASK_HOUR | INTERVAL_MASK_MINUTE)) != 0) {
         $self->ereport(ERROR,
               ERRCODE_SYNTAX_ERROR,
               "Time zone interval must be HOUR or HOUR TO MINUTE",
               $self->YYLLoc($interval, 6));
      }
      if (@$interval != 1 && defined $int) {
         $self->ereport(ERROR,
               ERRCODE_SYNTAX_ERROR,
   ...
   if (defined $int) {
      $t->typmods( $self->lappend(
         defined $interval ?
            ($interval, $self->makeIntConst($int, $self->YYLLoc($int, 3))) :
            ($self->makeIntConst(INTERVAL_FULL_RANGE, {}), $self->makeIntConst($int, $self->YYLLoc($int, 3)))
      ));
   }
   else {
      $t->typmods($interval);
   }
   return $self->makeStringConstCast($str, $self->YYLLoc($str, 3), $t);
}
sub got_zone_value_3 { $_[0]->makeAConst($_[1], $_[0]->YYLLoc($_[1], 1)) }

sub got_VariableResetStmt {
   SQL::Translator::Statement::VariableSet->new(
      name => $_[1],
   );
}
sub got_variable_cmd_2 { 'timezone' }
sub got_variable_cmd_3 { 'transaction_isolation' }
sub got_variable_cmd_4 { 'session_authorization' }
sub got_variable_cmd_5 { 'all' }

sub got_ConstraintsSetStmt {
   SQL::Translator::Statement::ConstraintsSet->new(
      constraints => $_[1]->[0],
      deferred    => $_[1]->[1] ? TRUE : FALSE,
   );
}

sub got_CheckPointStmt { SQL::Translator::Statement::CheckPoint->new() }

sub got_DiscardStmt { SQL::Translator::Statement::Discard->new( target => $_[1] ) }
...
sub got_DiscardStmt_all   { DISCARD_ALL   }
sub got_DiscardStmt_temp  { DISCARD_TEMP  }
sub got_DiscardStmt_plans { DISCARD_PLANS }
...
sub got_AlterTableStmt {
   SQL::Translator::Statement::AlterTable->new(
      relkind    => $_[1]->[0],
      missing_ok => $_[1]->[1] ? TRUE : FALSE,
      relation   => $_[1]->[2],
      cmds       => $_[1]->[3],
   );
}

sub got_AlterTableStmt_tbl  { (OBJECT_TABLE, @{$_[1]} ) }

sub got_AlterTableStmt_idx  { OBJECT_INDEX    }
sub got_AlterTableStmt_seq  { OBJECT_SEQUENCE }
sub got_AlterTableStmt_view { OBJECT_VIEW     }

sub got_alter_table_cmd { SQL::Translator::Statement::AlterTable::Command->new(@{$_[1]}) }

sub got_alter_table_cmd_s   { ( subtype => $_[1] ) }
sub got_alter_table_cmd_sn  { (
   subtype    => $_[1]->[0],
   name       => $_[1]->[1],
) }
sub got_alter_table_cmd_sd  { (
   subtype    => $_[1]->[0],
   def        => $_[1]->[1],
) }
sub got_alter_table_cmd_snd { (
   subtype    => $_[1]->[0],
   name       => $_[1]->[1],
   def        => $_[1]->[2],
) }
sub got_alter_table_cmd_smnb { (
   subtype    => $_[1]->[0],
   missing_ok => $_[1]->[1] ? TRUE : FALSE,
   name       => $_[1]->[2],
   behavior   => $_[1]->[3],
...
sub got_alter_table_cmd_s_1 { AT_AddOids         }
sub got_alter_table_cmd_s_2 { AT_DropOids        }
sub got_alter_table_cmd_s_3 { AT_EnableTrigAll   }
sub got_alter_table_cmd_s_4 { AT_EnableTrigUser  }
sub got_alter_table_cmd_s_5 { AT_DisableTrigAll  }
sub got_alter_table_cmd_s_6 { AT_DisableTrigUser }
sub got_alter_table_cmd_s_7 { AT_DropOf          }

sub got_alter_table_cmd_sd_1 { ( AT_AddColumn,       $_[1] ) }
sub got_alter_table_cmd_sd_2 { ( AT_AddConstraint,   $_[1] ) }
sub got_alter_table_cmd_sd_3 { ( AT_AddInherit,      $_[1] ) }
sub got_alter_table_cmd_sd_4 { ( AT_DropInherit,     $_[1] ) }
sub got_alter_table_cmd_sd_5 { ( AT_AddOf,           $_[0]->makeTypeNameFromNameList($_[1]) ) }
sub got_alter_table_cmd_sd_6 { ( AT_SetRelOptions,   $_[1] ) }
sub got_alter_table_cmd_sd_7 { ( AT_ResetRelOptions, $_[1] ) }
sub got_alter_table_cmd_sd_8 { ( AT_GenericOptions,  $_[1] ) }

sub got_alter_table_cmd_sn_1  { ( AT_DropNotNull,        $_[1] ) }
sub got_alter_table_cmd_sn_2  { ( AT_SetNotNull,         $_[1] ) }
sub got_alter_table_cmd_sn_3  { ( AT_ValidateConstraint, $_[1] ) }
sub got_alter_table_cmd_sn_4  { ( AT_ClusterOn,          $_[1] ) }
sub got_alter_table_cmd_sn_5  { ( AT_DropCluster,         NULL ) }
sub got_alter_table_cmd_sn_6  { ( AT_EnableTrig,         $_[1] ) }
sub got_alter_table_cmd_sn_7  { ( AT_EnableAlwaysTrig,   $_[1] ) }
sub got_alter_table_cmd_sn_8  { ( AT_EnableReplicaTrig,  $_[1] ) }
sub got_alter_table_cmd_sn_9  { ( AT_DisableTrig,        $_[1] ) }
sub got_alter_table_cmd_sn_10 { ( AT_EnableRule,         $_[1] ) }
sub got_alter_table_cmd_sn_11 { ( AT_EnableAlwaysRule,   $_[1] ) }
sub got_alter_table_cmd_sn_12 { ( AT_EnableReplicaRule,  $_[1] ) }
sub got_alter_table_cmd_sn_13 { ( AT_DisableRule,        $_[1] ) }
sub got_alter_table_cmd_sn_14 { ( AT_ChangeOwner,        $_[1] ) }
sub got_alter_table_cmd_sn_14 { ( AT_SetTableSpace,      $_[1] ) }

sub got_alter_table_cmd_snd_1  { ( AT_ColumnDefault,      @{$_[1]} ) }
sub got_alter_table_cmd_snd_2  { ( AT_SetStatistics,      $_[1]->[0], $_[1]->[1]+0 ) }
sub got_alter_table_cmd_snd_3  { ( AT_SetOptions,         @{$_[1]} ) }
sub got_alter_table_cmd_snd_4  { ( AT_ResetOptions,       @{$_[1]} ) }
sub got_alter_table_cmd_snd_5  { ( AT_SetStorage,         @{$_[1]} ) }
sub got_alter_table_cmd_snd_6  { (
   AT_AlterColumnType,
   $_[1]->[0],
   SQL::Translator::Statement::Column::Definition->new(
...
sub got_alter_table_cmd_snd_7  { ( AT_AlterColumnGenericOptions, @{$_[1]} ) }

sub got_alter_table_cmd_smnb_1 { ( AT_DropConstraint, @{$_[1]} ) }
sub got_alter_table_cmd_smnb_2 { ( AT_DropColumn,     @{$_[1]} ) }

sub got_opt_drop_behavior_1 { DROP_CASCADE  }
sub got_opt_drop_behavior_2 { DROP_RESTRICT }

sub got_opt_collate_clause {
   SQL::Translator::Statement::CollateClause->new(
      arg      => NULL,
      collname => $_[1],
...
sub got_reloption_elem_1 { $_[0]->makeDefElem        (@{$_[1]})                 }
sub got_reloption_elem_2 { $_[0]->makeDefElemExtended(@{$_[1]}, DEFELEM_UNSPEC) }
...
sub got_AlterCompositeTypeStmt {
   SQL::Translator::Statement::AlterTable->new(
      ...
      relation => $_[0]->makeRangeVarFromAnyName($_[1]->[0], $_[0]->YYLLoc($_[1]->[0], 1)),
      cmds     => $_[1]->[1],
      relkind  => OBJECT_TYPE,
   );
}

sub got_alter_type_cmd_add {
   SQL::Translator::Statement::AlterTable::Command->new(
      subtype    => AT_AddColumn,
      def        => $_[1]->[0],
      behavior   => $_[1]->[1],
   );
}
sub got_alter_type_cmd_drop {
   SQL::Translator::Statement::AlterTable::Command->new(
...
sub got_ClosePortalStmt { SQL::Translator::Statement::ClosePortal->new( portalname => $_[1] ) }
...
sub got_CopyStmt_1 {
   SQL::Translator::Statement::Copy->new(
      relation => $_[1]->[1],
      query    => NULL,
      attlist  => $_[1]->[2],
      is_from  => $_[1]->[4],
      ...
      options  => $_[0]->lappend(@{$_[1]->[0,3,6,8]}),
   );
}
sub got_CopyStmt_2 {
   SQL::Translator::Statement::Copy->new(
...
sub got_copy_from_from { TRUE  }
sub got_copy_from_to   { FALSE }

sub got_copy_opt_item_1  { $_[0]->makeDefElem("format",         "binary") }
sub got_copy_opt_item_2  { $_[0]->makeDefElem("oids",           TRUE);     }
sub got_copy_opt_item_3  { $_[0]->makeDefElem("delimiter",      $_[1]);    }
sub got_copy_opt_item_4  { $_[0]->makeDefElem("null",           $_[1]);    }
sub got_copy_opt_item_5  { $_[0]->makeDefElem("format",         "csv");    }
sub got_copy_opt_item_6  { $_[0]->makeDefElem("header",         TRUE);     }
sub got_copy_opt_item_7  { $_[0]->makeDefElem("quote",          $_[1]);    }
sub got_copy_opt_item_8  { $_[0]->makeDefElem("escape",         $_[1]);    }
sub got_copy_opt_item_9  { $_[0]->makeDefElem("force_quote",    $_[1]);    }
sub got_copy_opt_item_10 { $_[0]->makeDefElem("force_quote",    SQL::Translator::Statement::A_Star->new()) }
sub got_copy_opt_item_11 { $_[0]->makeDefElem("force_not_null", $_[1]);    }
sub got_copy_opt_item_12 { $_[0]->makeDefElem("encoding",       $_[1]);    }

sub got_opt_binary       { $_[0]->makeDefElem("format",    "binary") }
sub got_opt_oids         { $_[0]->makeDefElem("oids",      TRUE);     }
sub got_copy_delimiter   { $_[0]->makeDefElem("delimiter", $_[1]);    }

sub got_copy_generic_opt_elem { $_[0]->makeDefElem(@{$_[1]}) }
...
sub got_CreateStmt {
   SQL::Translator::Statement::Create->new(
      relpersistence => $_[1]->[0],
      if_not_exists  => $_[1]->[1] ? TRUE : FALSE,
      relation       => $_[1]->[2],
      constraints    => NIL,
      @{$_[1]->[3]},
   };
...
sub got_CreateStmt_1 { (
   ...
   options        => $_[1]->[2],
   oncommit       => $_[1]->[3],
   tablespacename => $_[1]->[4],
) }
sub got_CreateStmt_2 {
   my $ofTypename = $_[0]->makeTypeNameFromNameList($_[1]->[0]);
   $ofTypename->_set_location( $_[0]->YYLLoc($_[1]->[0], 1) );
...
(
   ...
   options        => $_[1]->[1],
   oncommit       => $_[1]->[2],
   tablespacename => $_[1]->[3],
) }

sub got_OptTemp_temp  { RELPERSISTENCE_TEMP      }
sub got_OptTemp_unlog { RELPERSISTENCE_UNLOGGED  }
...
sub got_columnOptions {
   ...
   $_[0]->SplitColQualList($_[1]->[1], $n);
   return $n;
}

sub got_ColConstraint_1 {
   $_[1]->[2]->conname($_[1]->[1]);
   $_[1]->[2]->_set_location($_[0]->YYLLoc($_[1]->[0], 1));
   return $_[1]->[2];
}
...
sub got_ColConstraintElem {
   SQL::Translator::Statement::Constraint->new(
      @{$_[1]},
      location => $_[0]->YYLLoc($_[1], 1),
   )
}
...
sub got_ColConstraintElem_7 { (
   ...
   pk_attrs        => $_[1]->[1],
   fk_matchtype    => $_[1]->[2],
   fk_upd_action   => $_[1]->[3]{upd} || FKCONSTR_ACTION_NOACTION,
   fk_del_action   => $_[1]->[3]{del} || FKCONSTR_ACTION_NOACTION,
   skip_validation => FALSE,
   initially_valid => TRUE,
) }

sub got_ConstraintAttr {
   SQL::Translator::Statement::Constraint->new(
      contype  => $_[1],
      location => $_[0]->YYLLoc($_[1], 1),
   )
}
sub got_ConstraintAttr_1 { CONSTR_ATTR_DEFERRABLE     }
sub got_ConstraintAttr_2 { CONSTR_ATTR_NOT_DEFERRABLE }
sub got_ConstraintAttr_3 { CONSTR_ATTR_DEFERRED       }
sub got_ConstraintAttr_4 { CONSTR_ATTR_IMMEDIATE      }

sub got_TableLikeClause {
   SQL::Translator::Statement::TableLikeClause->new(
      relation => $_[1]->[0],
      options  => $_[1]->[1],
   );
}

sub got_TableLikeOptionList   { reduce { $a | $b }  @{$_[1]} }
sub got_TableLikeOptionList_i { reduce { $a | $b }  @{$_[1]} }
sub got_TableLikeOptionList_e { reduce { $a & ~$b } @{$_[1]} }
...
sub got_TableConstraint_1 {
   $_[1]->[1]->conname($_[1]->[0]);
   $_[1]->[1]->_set_location($_[0]->YYLLoc($_[1], 1));
   return $_[1]->[1];
}

sub got_ConstraintElem {
   # [0] == contype
   # [1] == Constraint keypairs
   # [2] == CASbits terms
   # [3] == CASbits flags
   ...
   $_[0]->processCASbits($_[1]->[2], $_[0]->YYLLoc($_[1]->[2], 5), $_[1]->[0], $n, @{$_[1]->[3]});
   $n->initially_valid( !$n->skip_validation )
      if ($_[1]->[0] eq CONSTR_CHECK || $_[1]->[0] eq CONSTR_FOREIGN);
   return $n;
}
...
sub got_ConstraintElem_7 { [
   ...
   [
      ...
      pk_attrs      => $_[1]->[2],
      fk_matchtype  => $_[1]->[3],
      fk_upd_action => $_[1]->[4]{upd} || FKCONSTR_ACTION_NOACTION,
      fk_del_action => $_[1]->[4]{del} || FKCONSTR_ACTION_NOACTION,
   ],
   $_[1]->[5], [1,1,1,0]
] }
...
sub got_ExclusionConstraintElem {
   push @{$_[1]->[0]}, $_[1]->[1];
   return $_[1]->[0];
}
...
sub got_key_update { [ upd => $_[1] ] }
sub got_key_delete { [ del => $_[1] ] }
...
sub got_OptWith_2 { [ defWithOids(TRUE)  ] }
sub got_OptWith_3 { [ defWithOids(FALSE) ] }
...
sub got_SignedIconst { $_[1]->[0] ? eval $_[1]->[0].$_[1]->[1] ? $_[1]->[1] }
